...
class _NewExpenseState extends State<NewExpense> {
  ...
  // FUNGSI: Menampilkan dialog error sesuai platform (iOS atau Android).
  void _showDialog() {
    // PENGECEKAN PLATFORM: Jika aplikasi berjalan di iOS...
    if (Platform.isIOS) {
      // ...tampilkan dialog dengan gaya iOS (Cupertino).
      showCupertinoDialog(
          context: context,
          builder: (ctx) => CupertinoAlertDialog(
                ...
                actions: [
                  // WIDGET SPESIFIK iOS: Gunakan CupertinoDialogAction untuk tombol yang sesuai dengan gaya iOS.
      ...
      // PENGECEKAN PLATFORM: Jika bukan iOS (misal: Android)...
      // ...tampilkan dialog dengan gaya Android (Material).
      showDialog(
        context: context,
        builder: (ctx) => AlertDialog(
          ...
          content: const Text(
              'Please make sure a valid title, amount, date, and category was entered.'),
          actions: [
            // WIDGET SPESIFIK ANDROID: TextButton adalah pilihan yang tepat untuk Material.
            TextButton(
              onPressed: () {
                Navigator.pop(ctx); // Tutup dialog.
              },
  ...
  void _submitExpenseDate() {
    ...
    // KONDISI VALIDASI: Variabel ini bernilai true jika input bukan angka ATAU angkanya negatif.
    final amountIsInvalid = enteredAmount == null || enteredAmount < 0;

    // BLOK VALIDASI UTAMA: Mengecek semua kondisi sebelum menyimpan.
    if (_titleControler.text.trim().isEmpty ||
        amountIsInvalid ||
        _selectedDate == null) {
      // MEMANGGIL FUNGSI: Jalankan fungsi _showDialog() untuk menampilkan pesan error.
      // Tanda '()' di akhir diperlukan untuk mengeksekusi fungsi.
      _showDialog();
      // MENGHENTIKAN FUNGSI: 'return' memastikan kode di bawahnya tidak akan dieksekusi jika validasi gagal.
      return;
    }

    // INFO: Jika semua data valid, panggil fungsi 'onaddexpense' dari widget induk untuk mengirim data.
    // Tanda seru (!) pada _selectedDate! memberitahu Dart bahwa kita yakin variabel ini tidak null saat ini.
  ...
  // FUNGSI: Lifecycle method yang dipanggil saat widget akan dihapus dari tree.
  @override
  void dispose() {
    // PENTING: Selalu dispose controller untuk melepaskan memori dan mencegah kebocoran (memory leak).
